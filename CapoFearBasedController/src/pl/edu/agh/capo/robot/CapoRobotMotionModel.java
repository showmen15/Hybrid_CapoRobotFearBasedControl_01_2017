...
public class CapoRobotMotionModel
{
	...
	// /
	// / If physical constraints are exceeded, linear velocity will be lowered,
	// arc radius should be preserved.
	// /
	// / </summary>
	...
	// / <summary>
	// / Makes robot go along an arc of specified arcRadius with specified
	// linearVelocity.
	// /
	// / If physical constraints are exceeded, linear velocity will be lowered,
	// arc radius should be preserved.
	// /
	// / Equations are:
	// / 1) arcRadius = wheelsHalfDistance * (rightSpeed + leftSpeed) /
	...
	// /
	// / Equations are:
	// / 1) linearVelocity / angularVelocity = wheelsHalfDistance * (rightSpeed
	// + leftSpeed) / (rightSpeed - leftSpeed) - compare
	// Move_LinearVelocity_ArcRadius
	// / 2) linearVelocity = (rightSpeed + leftSpeed) / 2
	// / which give:
	// / leftSpeed = linearVelocity * (1 + ((angularVelocity *
	// wheelsHalfDistance) / linearVelocity))
	// / rightSpeed = linearVelocity * (1 - ((angularVelocity *
	// wheelsHalfDistance) / linearVelocity))
	// / </summary>
	// / <param name="linearVelocity"></param>
	// / <param name="angularVelocity"></param>
	...
	public boolean setVelocity_LinearVelocity_AngularVelocity(double linearVelocity, double angularVelocity)
	{
		...
		{
			// velocityLeft = linearVelocity * (1 + ((angularVelocity *
			// wheelsHalfDistance) / linearVelocity));
	...
	public boolean setVelocity_LinearVelocity_AngularVelocity_Without_wheelsHalfDistance(double linearVelocity, double angularVelocity)
	{
		if (Math.abs(linearVelocity) < verySmallDouble)
		{
			return setVelocity_TurnAround(angularVelocity);
		...
		else
		{
			// velocityLeft = linearVelocity * (1 + ((angularVelocity *
			// wheelsHalfDistance) / linearVelocity));
			velocityLeft = linearVelocity + angularVelocity;
			// velocityRight = linearVelocity * (1 - ((angularVelocity *
			// wheelsHalfDistance) / linearVelocity));
	...
	// / which give:
	// / leftSpeed = (arcRadius * angularVelocity) * (1 + (wheelsHalfDistance /
	// arcRadius))
	// / rightSpeed = (arcRadius * angularVelocity) * (1 - (wheelsHalfDistance /
	// arcRadius))
	// / </summary>
	// / <param name="arcRadius"></param>
	// / <param name="angularVelocity"></param>
	...
	public boolean setVelocity_ArcRadius_AngularVelocity(double arcRadius, double angularVelocity)
	{
		...
		{
			// velocityLeft = (arcRadius * angularVelocity) * (1 +
			// (wheelsHalfDistance / arcRadius));
			velocityLeft = angularVelocity * (arcRadius + wheelsHalfDistance);
			// velocityRight = (arcRadius * angularVelocity) * (1 -
			// (wheelsHalfDistance / arcRadius));
	...
	/**
	 * First calculate the Center of the circle: arcCenter = [x − R sin(θ) , y +
	 * R cos(θ)] Then calculate new location: rotating a distance R about its
	 * ICC with an angular velocity of ω.
	 * 
	 * http://chess.eecs.berkeley.edu/eecs149/documentation/differentialDrive.
	 * pdf
	 * 
	 * @param deltaTime
	 *            in seconds
	...
	public Location getLocationAfterTime(double deltaTime)
	{
		...
		{
			return new Location(location.positionX + getLinearVelocity() * Math.cos(location.direction * deltaTime), location.positionY + getLinearVelocity() * Math.sin(location.direction * deltaTime), location.direction);
		}
		else
		{
			...
			double newX = Math.cos(angularVelocityDeltaTime) * (location.positionX - arcCenterX) - Math.sin(angularVelocityDeltaTime) * (location.positionY - arcCenterY) + arcCenterX;
			double newY = Math.sin(angularVelocityDeltaTime) * (location.positionX - arcCenterX) + Math.cos(angularVelocityDeltaTime) * (location.positionY - arcCenterY) + arcCenterY;
	...
	// /
	// / TODO - cannot just restrict wheels velocity, linear v of the robot is
	// to be restricted
	// / </summary>
	// / <returns></returns>
