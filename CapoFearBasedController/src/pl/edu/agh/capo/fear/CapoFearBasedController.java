...
public class CapoFearBasedController implements Runnable
{
	...
	public CapoFearBasedController(int robotId, double maxLinearVelocity, String mazeRosonFilename) throws Exception
	{
	...
	public void run()
	{
		...
		while (this.isRun)
		{
			...
			else
			{
				...
				this.capoRobotMotionModel.setVelocity_LinearVelocity_AngularVelocity(Math.cos(angleToTarget / 2) * Math.min(1, destinationDistance + 0.1) * this.capoRobotMotionModel.getMaxLinearVelocity() / 2, Math.sqrt(3 * Math.abs(angleToTarget)) * Math.signum(angleToTarget));
			}
			...
			if (this.capoRobotMotionModel.getLinearVelocity() > 0 && getFrontDistance() < 0.45)
			{
				this.capoRobotMotionModel.setVelocity(0, 0);
